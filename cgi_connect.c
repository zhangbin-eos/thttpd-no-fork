...
int cgi_connect_thttpd_open(char *objname, void *arg)
{
	...
	strncpy(&(objaddr.sun_path[0]), objname,
		(sizeof(objaddr.sun_path) - 1));

	if (connect(fd, (struct sockaddr *)&objaddr, sizeof(struct sockaddr_un))
	    < 0)
	{
		pr_log(LOG_ERR, "connect -->%s error,%s", objaddr.sun_path,
		       strerror(errno));
		return -1;
	}
	return fd;
...
int cgi_connect_cgi_daemon_open(char *selfname, void *arg)
{
	...
	strncpy(&(selfaddr.sun_path[0]), selfname,
		(sizeof(selfaddr.sun_path) - 1));

	svfd = socket(AF_UNIX, SOCK_STREAM, 0);	// liu式套接字

	if (remove(selfname) == -1 && errno != ENOENT)
	{
		pr_log(LOG_ERR, "remove-%s error:%s", selfname,
		       strerror(errno));
		return -1;
	}

	if (bind(svfd, (struct sockaddr *)&selfaddr, sizeof(struct sockaddr_un))
	    == -1)
	{
	...
	if (listen(svfd, 32) == -1)
	{
		pr_log(LOG_ERR, "listen %s error:%s", selfname,
		       strerror(errno));
		return -1;
	}
...
int cgi_connect_cgi_daemon_accept(int fd, void *arg)
{
